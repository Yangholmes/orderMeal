...
yangImgInput.prototype = {
	...
	html: 
			// '<div id="{yang-img-input-id}" class="yang-img-input">' + 
				'<div id="{yang-img-input-id}-touch-layout" class="yang-img-input-touch-layout">' + 
					...
					'<img id="{yang-img-input-id}-preview" class="yang-img-input-preview" src={yang-img-input-preview} alt="image preview here~">' + 
				'</div>' + 
	...
	style: {
				...
				'yang-img-input-mask': 'margin: 0; position: absolute; transition: background-color .5s;', 
				'yang-img-input-message': 'margin: 0; color: rgba(220, 220, 220, 0); position: absolute; text-align: center; transition: color .5s;',  
				'yang-img-input-preview': 'margin: 0; width: auto; height: auto; min-width: 800px;', 
				'yang-img-input-choose': 'margin: 0; position: absolute; width: 0; visibility: hidden;',
				//pseudo class :hover
	...
	_createHTML: function(){
		...
		that.html = that._replace(that.html, '{yang-img-input-preview}', that.img); //replaceId
	...
	_regEvents: function(){
		...
		touchLayout.onmouseover = function(){
			var imgHeight = img.offsetHeight,
				...
				mask.style.height = imgHeight + 'px'; mask.style.width = imgWidth + 'px';
				message.style.width = imgWidth + 'px'; message.style.height = imgHeight + 'px'; message.style.lineHeight = imgHeight + 'px';
		};
	...
	setImg: function(imgSrc){
		if( imgSrc && typeof imgSrc == 'string' ){
			var touchLayout = this.self.getElementsByClassName('yang-img-input-preview')[0];
			touchLayout.src = imgSrc;
		}
	},
